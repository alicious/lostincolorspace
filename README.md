...
1. Retrieve the chip from where it is docked, and optionally choose to undo the   
   previous player's move by clicking once  
...
*Memory Challenge:*  
At the end of setup, once all players are ready, hide goals for the remainder of the game  
  
  
### Colorful Language ###  
  
*3+ players (with 2 player variant)*  
*2 blinks per player, minus one*  
...
PLAYING WITH FEWER BLINKS:  
Play with one paint chip per player, and pass a primary around until everyone  
has made a color to test.  
...
*2-4 players*  
*18 blinks*  
///// currently tested for two players with 18 blinks /////  
///// need to playtest other player and blink counts /////  
...
* All players discard one goal of their choice.   
* Convert all discarded goals and any remaining blinks to blank paint chips to make a   
  starting chip pool.  
  
GAME PLAY:  
Players take turns in clockwise order, taking one action per turn.  
   
  
**Phase 1**   
In Phase one, players take turns adding a chip from the pool to the board, optionally   
drawing color from one adjacent face before rotating into final position.  
(This is equivalent to the "Place" action in Phase 2, below).   
  
After the chip pool is emptied for the first time, start Phase 2.   
  
**Phase 2**  
Until the game end is triggered (see below), players perform exactly one of the following  
...
**No Random Goals**  
At the start of setup, players make the starting goals manually, keeping goals  
secret from other players. Once every player has made the amount listed in the setup  
table, shuffle and draw the required amount. Please note that it can break the game if  
you make goals that are too close to the primary colors, so how about don't.   
  
**Team Play (4 players)**  
Form two teams of two, and sit across from your partner. At the end of setup, write a   
codename for your goal color on a piece of paper, and pass it to your partner. Play  
proceeds as above, except you can additionally attempt to dock at your partner's goal,  
and you only dock three chips at a goal (as in the two player game).   
The team with the highest combined point total wins the game.  
  
  
### MAKE UP YOUR OWN GAME <3 ###  
*some players*  
*some blinks*  
